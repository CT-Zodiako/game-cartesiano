...
test('room snapshots serialize initial timestamps as null and ids per engine', () => {
  const firstEngine = new RoomEngine({ now: () => 1_000, setTimer: () => 1, clearTimer: () => {} });
  ...
  const secondEngine = new RoomEngine({ now: () => 1_000, setTimer: () => 1, clearTimer: () => {} });
...
test('non-host cannot start game', () => {
  ...
  const started = engine.startGame({ roomId: created.roomId, actorPlayerId: joined.playerId });
...
test('accepted claim returns ordered ranking and bumps version', () => {
  ...
  const room = engine.roomsById.get(created.roomId);
  const roundId = room?.currentRound ?? 1;
  const target = room?.targetsByRoundPlayer.get(`${roundId}:${created.hostId}`) ?? { x: 0, y: 0 };
  const result = engine.submitClaim({ roomId: created.roomId, roundId, playerId: created.hostId, target, serverReceivedAtMs: clock.now(), wsConnectionSeq: 1 });
