...
import { WSClient, handleRoundStarted, handleClaimAck, handleLateAlert, handleRankingUpdated, handleError, handleGameEnded } from '@infrastructure/ws/client.ts';
import type { RoomState, RankingEntry, RoundStartedEvent, GameEndedEvent } from '@infrastructure/ws/events.ts';
...
const leaveRoomBtn = document.getElementById('btn-leave-room') as HTMLButtonElement | null;
...
const playerNameInputCreate = document.getElementById('player-name-create') as HTMLInputElement | null;
const playerNameInputJoin = document.getElementById('player-name-join') as HTMLInputElement | null;
const roomCodeInputJoin = document.getElementById('room-code-join') as HTMLInputElement | null;

// Botones de confirmar
const confirmCreateBtn = document.getElementById('btn-confirm-create') as HTMLButtonElement | null;
const confirmJoinBtn = document.getElementById('btn-confirm-join') as HTMLButtonElement | null;

// Botones atrás
const backCreateBtn = document.getElementById('btn-back-create') as HTMLButtonElement | null;
const backJoinBtn = document.getElementById('btn-back-join') as HTMLButtonElement | null;
...
function showOnlinePanel(panel: HTMLElement | null): void {
  if (!onlineEntryMenu || !onlineCreatePanel || !onlineJoinPanel || !onlineLobby) return;
...
// Config inputs
const configMaxPlayers = document.getElementById('config-max-players') as HTMLInputElement | null;
const configRounds = document.getElementById('config-rounds') as HTMLInputElement | null;
const configSeconds = document.getElementById('config-seconds') as HTMLInputElement | null;
const configMaxXy = document.getElementById('config-max-xy') as HTMLInputElement | null;

const roomInfoText = null; // Eliminado del HTML

const challengeText = null; // Eliminado del HTML
...
function syncLobbyUi(roomState: RoomState): void {
  ...
  const self = (roomState.players ?? []).find(
    (p) => p.name === playerNameInputCreate?.value?.trim() || p.name === playerNameInputJoin?.value?.trim() || p.playerId === onlineState.playerId,
  );
  ...
  const playersAsRanking: RankingEntry[] = (roomState.players ?? []).map(p => ({
    playerId: p.playerId,
    name: p.name,
  ...
  if (startGameBtn) {
    ...
    if (isHost) {
      ...
      startGameBtn.disabled = !(roomState.status === 'LOBBY' && (roomState.players ?? []).length >= 2);
    } else {
...
function initOnlineMode(): void {
  ...
  ws.on('ROOM_SNAPSHOT', (event) => {
    const roomState = (event as { roomState: RoomState }).roomState;
    const yourPlayerId = (event as { yourPlayerId?: string }).yourPlayerId;
  ...
  handleClaimAck(ws, (event) => {
    const points = (event as { pointsEarned?: number }).pointsEarned;
    if (event.status === 'ACCEPTED') {
      feedbackText!.textContent = `✅ Correcto! +${points} puntos`;
  ...
  handleGameEnded(ws, (event: GameEndedEvent) => {
    showFinalRankingModal(event.ranking || []);
  });
  ...
  leaveRoomBtn?.addEventListener('click', () => {
    ...
    onlineState.roomId = undefined;
    onlineState.playerId = undefined;
    onlineState.currentRound = undefined;
    onlineState.deadlineMs = undefined;
    onlineState.ranking = [];
    onlineMenuState = 'menu';
...
function showFinalRankingModal(ranking: RankingEntry[]): void {
  ...
  const rankingListEl = modal.querySelector('#final-ranking-list') as HTMLElement;
  if (rankingListEl) {
