...
export function computeRanking(
	players: Map<string, RankingPlayer> | RankingPlayer[],
): RankingEntry[] {
	...
	return list
		.map((player) => ({
			...
			lastAcceptedAtMs:
				...
				Number.isFinite(player.lastAcceptedAtMs)
					? player.lastAcceptedAtMs
					: null,
		}))
		.sort(
			(a, b) =>
				b.totalScore - a.totalScore || a.playerId.localeCompare(b.playerId),
		);
}
